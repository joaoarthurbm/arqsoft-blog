...
### Visão de Informação
<img src="linux/kernel_informacao.png"></img>
**obs:** épsilon representa uma transição automática.

No kernel do linux, as informações que o sistema manipula são: chamadas ao sistema, solicitações entre um módulo e outro para atender essas chamadas e por fim, as respostas a essas chamadas. Inicialmente temos o kernel aguardando chamadas ao sistema, até que uma aplicação faz uma chamada e o kernel passa para o estado "processando system call para solicitar ao hardware". Se essa chamada ao sistema gerar uma exceção, uma resposta de falha é enviada para a aplicação que fez a chamada ao sistema e o kernel volta para o estado "aguardando system calls". Por outro lado, se a chamada ao sistema não gerar uma exceção, o estado do kernel muda para "hardware executando instruções solicitadas". Uma vez que o hardware execute as instruções que foram solicitadas, o kernel passará para o estado "processando resposta para a aplicação" e ao finalizar esse processo, a resposta de sucesso será enviada para a aplicação que fez a chamada ao sistema. E o kernel voltará para o estado "aguardando system calls", para atender mais solicitações.
